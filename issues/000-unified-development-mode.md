...
## 架构铁律：Core 与 Platform 分离

### 核心原则

**@agentvm/core = 抽象层（平台无关）**

- 只定义类型（interface, type）
- 只定义抽象接口（Repository, Service interface）
- 零具体实现
- 零平台依赖（不依赖 SQLite, D1, HTTP 框架等）

**@agentvm/avm = 实现层（本地平台）**

- 实现 core 定义的接口
- 具体存储实现（SQLiteTenantRepository 等）
- HTTP/WebSocket 路由
- CLI 入口
...
┌────────────────────────────────────────────┐
│                                            │
│  @agentvm/avm          @agentvm/cloud      │
│  (本地/开源)            (云平台/未来)       │
│                                            │
│  - SQLite              - D1/KV/R2          │
│  - 本地 HTTP           - Cloudflare Workers│
│  - 单机运行            - 分布式部署         │
│                                            │
└────────────────────────────────────────────┘
```
...
```typescript
// ✅ core: 只定义接口
// packages/core/src/tenant/TenantRepository.ts
...
// ✅ avm: 实现接口
// packages/avm/src/repositories/SQLiteTenantRepository.ts
import { TenantRepository } from "@agentvm/core";
...
// ✅ 未来 cloud: 实现同样的接口
// packages/cloud/src/repositories/D1TenantRepository.ts
import { TenantRepository } from "@agentvm/core";
...
1. **core 纯净** - 只有类型和接口，可被任何平台复用
2. **实现可换** - SQLite → D1 → PostgreSQL，不改 core
3. **一次开发，到处运行** - 同一套 core，多套实现
4. **渐进式迁移** - 先开发 avm，稳定后再开发 cloud

### 铁律检查清单

开发时自问：

- [ ] 这个类型/接口应该放 core 还是 avm？
- [ ] core 里有没有 import 具体实现？
